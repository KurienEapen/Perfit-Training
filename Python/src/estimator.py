...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240)):	# passing the model along with the size information default size is 320,240
        self.target_size = target_size				# setting the size to self.target_size
    ...
    @staticmethod
    def draw_humans(npimg, humans, imgcopy=False): # input frame and human position index values matrix
        ...
        for human in humans: # detecting total no of humans
            # draw point
            for i in range(common.CocoPart.Background.value):	# 18 body points
                if i not in human.body_parts.keys():	#only visible points available			
                    continue
				# passing only visible points 
                body_part = human.body_parts[i]			# body part point marking
                print(body_part)

                center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))	# [x,y]				cordinate to plot the circle inside the human body
                print(center)

                centers[i] = center
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=10, lineType=8, shift=0)
                #cv2.imshow('outimage',npimg)
                #cv2.waitKey(100)
                #time.sleep(10)
            # draw line
