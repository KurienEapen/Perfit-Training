...
def get_graph_path(model_name):
    dyn_graph_path = {
        'cmu': './models/graph/cmu/graph_opt.pb',				# dont know the urpose of the graph path
...
def model_wh(resolution_str):
    width, height = map(int, resolution_str.split('x'))				# getting width and height with 'x' used to split the data
    if width % 16 != 0 or height % 16 != 0:					# width and height must be multiple of 16
